                        ...
                        <tbody>
                            {data.items.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="p-2 text-end">Php {Number(item.amount)?.toFixed(2)}</td>
                                    <td className="p-2 text-end">Php {(Number(item.qty) * Number(item.amount)).toFixed(2)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="d-flex justify-content-end">
                    <div className="p-3 w-100 totals-box" style={{maxWidth : "300px"}}>
                        <div className="d-flex justify-content-between mb-2">
                            <span>Subtotal: </span>
                            <span>Php {data.subtotal.toFixed(2)}</span>
                        </div>

                        {data.tax > 0 && (
                            <div className="d-flex justify-content-between mb-2">
                                <span>Tax ({data.tax}%):</span>
                                <span>Php {data.taxAmount.toFixed(2)}</span>
                            </div>
                        )}
